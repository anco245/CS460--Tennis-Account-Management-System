...
public class Court6Controller implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    ...
    try (Connection connection = DriverManager.getConnection(Database.url, Database.username, Database.password)) {
      ...
      PreparedStatement preparedStatement = connection.prepareStatement("select * from court6");
      ...
      while (resultSet.next()) {
        ...
        if(occ == 0)
        {
          person = new Person(t.toString().substring(0, 19), "Available", 6, Database.reservedGuests);
        } else {
          person = new Person(t.toString().substring(0, 19), "Already Taken", 6, Database.reservedGuests);
        }
