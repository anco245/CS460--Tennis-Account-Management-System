...
public class Court10Controller implements Initializable {
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    ...
    try (Connection connection = DriverManager.getConnection(Database.url, Database.username, Database.password)) {
      ...
      PreparedStatement preparedStatement = connection.prepareStatement("select * from court10");
      ...
      while (resultSet.next()) {
        ...
        if(occ == 0)
        {
          person = new Person(t.toString().substring(0, 19), "Available", 10, Database.reservedGuests);
        } else {
          person = new Person(t.toString().substring(0, 19), "Already Taken", 10, Database.reservedGuests);
        }
